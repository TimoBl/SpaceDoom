...
io.on('connection', function(socket){
  ...
  socket.on('start_game', function(name) {
    if (name.length > 0 && name.length <= 10){
      ...
      var p = {name: name, id: socket.id, x:nx , y: ny, rot: -Math.PI / 2, vx: 0, vy: 0, r: 50, kills: 0, killed: 0, health: 100, color: get_random_color(), shoot_countdown: 0, input_r: 0}
      players.push(p)
  ...
  socket.on('change_y_input', function(vy) {
    var player = players.filter(obj => {
      return obj.id === socket.id
    })
    player[0].vx += vy * Math.cos(player[0].rot) * 0.5
    player[0].vy += vy * Math.sin(player[0].rot) * 0.5
...
function shoot(player){
  ...
  var bullet = {x: player.x, y: player.y, r:5, vx: vx, vy: vy, id: player.id, state:"shot"}
  bullets.push(bullet)
}
  ...
  var logic = `
    <script type="text/javascript">
      ...
      function display_player_rank(players){
        ...
        for (var i = 0; i < players.length; i++){
          text += "<tr><td>" + players[i].name + "</td><td>" + players[i].kills.toString() + "</td><td>" + players[i].killed.toString() + "</td><tr>"
        }
        text += "</table>"
